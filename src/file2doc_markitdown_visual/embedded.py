    ...
    def parse(
        ...
        *,
        ...
        if cached is not None:
            if self._artifact_collector is not None and cached.source_ref is not None:
                self._artifact_collector.add_occurrence(
                    cached.source_ref,
                    location=location,
                )
            return cached
        ...
        try:
            ...
            result = EmbeddedVisualResult(
                ...
                source_ref=f"source_image_sha256:{normalized_hash}",
            )
...
def render_embedded_visual(result: EmbeddedVisualResult, *, location: str) -> str:
    ...
    if result.warning is not None:
        ...
        lines.append(_demote_headings(result.markdown, levels=1))
    return "\n".join(lines).strip()
